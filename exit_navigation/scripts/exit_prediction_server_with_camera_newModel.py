...
from VAE_model_new import Conv2D_Encoder_Decoder, Conditional_VAE
...
def load_model(model_name,input_size=128,latent_dim=64,multi_task_Flag=True):
    channel_dimList = [32,32,64,128]
    vae_model=Conv2D_Encoder_Decoder(input_size=input_size,latent_dim=latent_dim,\
        layerList=channel_dimList,sampling_layer_Flag=False,\
            multi_task_Flag=multi_task_Flag,camera_Flag=True,skip_type='concat')
    unet = Conditional_VAE(vae_model=vae_model,\
        sampling_layer_Flag=False,multi_task_Flag=multi_task_Flag,
                        exit_loss_weight=1.0)
    filepath = path.join(path.dirname(__file__), "..","learning","model","camera",model_name,'base_model_weights')
...
def exit_driven_path_plan(unet,laser_input_global,segmented_map,laser_input,camera_input,camera_channel,centroid_size=7,grid_size=1,robot_radius=0.1,robot_pose=[0,0],show_animation=False,plot_dir=[],fig_idx=[],exit_round_cnt=0,multiTaskFlag = True):
    ...
    if multiTaskFlag:
        ...
        z_mean, z_log_var, z,exit_build = unet.encoder_decoder(x_input,method=1)
...
#     print(exit_build[exit_loc[0]-filter_half_size:exit_loc[0]+filter_half_size+1,exit_loc[1]-filter_half_size:exit_loc[1]+filter_half_size+1].shape)
    ...
    return laser_input,x_input,exit_build,exit_loc,prob,region_max_prob,region_avr_prob,exit_loc_nearest,Astar_traj,Astar_traj_remapped,Goalpoint,Astar_rplan_Flag, Astar_openSet_Flag
...
def handle_exit_request(req):
    ...
    model_typle = 'conv2D'
    latent_dim = 256
    # vae_model_name = Conditional_New_VAE#Conditional_ResNet_VAE#Conditional_New_VAE#multiModal_VAE #Camera_VAE #Conditional_New_VAE
    model_name = model_typle + 'rplan_cameraTrue' + \
        '_u-net_latent'+str(latent_dim)+'_multiTask'+str(multi_task_Flag) + \
            '_transfer'+str(transfer_flag)
    #vae_model_path = 'checkpoint_channel4_exitWeight10Layerdoubled/' + model_name 
    unet = load_model(model_name,input_size=128,latent_dim=latent_dim,multi_task_Flag=multi_task_Flag)
    camera_channel = 1
    robotPose = [roboGrid[0], roboGrid[1]]#maybe need to be grid
    #Might need to format map
    laser_input,x_input,exit_build,exit_loc,prob,region_max_prob,region_avr_prob,exit_loc_nearest,Astar_traj,Astar_traj_remapped,Goalpoint,Astar_rplan_Flag, Astar_openSet_Flag = exit_driven_path_plan(unet,shaped_map,segmented_map,threshmap,camera_input,camera_channel,robot_pose=robotPose, multiTaskFlag=multi_task_Flag)
    ...
    axs_astar[1].imshow(exit_build[0,:,:,0],cmap='jet')#vmax=1.0,vmin=0,
