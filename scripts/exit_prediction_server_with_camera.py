...
def find_nearest_reachable_point_from_exit(map_input,exit_loc,camera_map,filter_size=9):
    ...
    print(np.unique(map_input),' map input uniques')
    print(np.unique(unknown_space_map),'unique of unkown space map ',map_color['uncertain'])
...
def exit_driven_path_plan(unet,laser_input_global,segmented_map,laser_input,camera_input,camera_channel,centroid_size=7,grid_size=1,robot_radius=0.1,robot_pose=[0,0],show_animation=False,plot_dir=[],fig_idx=[],exit_round_cnt=0):
    ...
    print(np.unique(laser_input),'laser input unique')
    if camera_channel==3:
    ...
    print(exit_loc,'deep learned exit loc')
...
def handle_exit_request(req):
    ...
    fig_rotation = plt.figure(2)
    axs_rotation = []
    axs_rotation.append(fig_rotation.add_subplot(1,2,1))
    axs_rotation.append(fig_rotation.add_subplot(1,2,2))
    axs_rotation[0].cla()
    axs_rotation[0].title.set_text('before rotation')
    axs_rotation[0].imshow(shaped_map,cmap="gray")
    # axs_astar[0].invert_yaxis()
    axs_rotation[0].plot(roboGrid[0], roboGrid[1], "xr",markersize=6,label='robot location.')
    #axs_rotation[0].plot(64, 64, "og",markersize=6,label='init_pose')
    #axs_rotation[0].plot(exit_loc_nearest[0], exit_loc_nearest[1],"Dr",markersize=5,label='goal')
    #axs_rotation[0].plot(Goalpoint[0], Goalpoint[1],"or",markersize=5,label='goalUsed')
    ...
    for i in range(n):
        ...
        fimg = cv2.filter2D(src=image_normalize(shaped_map), ddepth=-1, kernel=kernal)
        retval, threshed[i] = cv2.threshold(fimg,0.75,1,cv2.THRESH_BINARY)
    ...
    print('Gabor best rotation is ', bestFilter, ' degrees')
    print('The responses were ',responses)
    #bestFilter = 40
    print(np.unique(shaped_map),'shaped map before rotation')
    shaped_map=exit_prediction_utils.rotate_img(shaped_map,bestFilter)
    print(np.unique(shaped_map),'shaped map after rotation')
       

    #shaped_map = imutils.rotate(shaped_map.astype(int), angle=bestFilter)
    #pillowImage = Image.fromarray(shaped_map)
    #pillowImage = pillowImage.rotate(bestFilter)
    #shaped_map = np.asarray(pillowImage)
    #rotate robot position to match rotated map
    center=np.array([shaped_map.shape[0]/2.,shaped_map.shape[1]/2.])
    roboCenter = roboGrid-center
    print(roboCenter,'centered robot coord')
    roboGrid = np.matmul(roboCenter,rotationMatrix(bestFilter))

    #roboGrid = np.matmul(roboGrid,rotationMatrix(bestFilter))
    print(roboGrid,'robogrid')
    roboGrid = roboGrid+center
    roboGrid = roboGrid.astype(int)
    print(roboGrid,'int robogrid')
    #segment map
    ...
    axs_rotation[0].axis('off')
    axs_rotation[1].cla()
    axs_rotation[1].title.set_text('after rotation')
    axs_rotation[1].imshow(shaped_map,cmap='gray')
    # axs_astar[1].invert_yaxis()
    axs_rotation[1].plot(roboGrid[0], roboGrid[1], "xr",markersize=6,label='robot location.')
    #axs_rotation[1].plot(exit_loc_nearest[0], exit_loc_nearest[1],"Dr",markersize=5,label='goal')
    #axs_rotation[1].plot(Goalpoint[0], Goalpoint[1],"or",markersize=5,label='goalUsed')
    #axs_rotation[0].plot(nonExitPoints[1][:],nonExitPoints[0][:],'ow',markersize=1,alpha=.25,label='non-exits')
    #axs_rotation[1].plot(64,64,'og',markersize=5)
    axs_rotation[1].axis('off')
    ...
    print(np.unique(segmented_map),'values of local (segmented) map')
    ...
    print(np.unique(threshmap),'unique values of thresholded local (segmented) map')
    print(np.unique(threshGlobal),'unique values of thresholded global map')
    ...
    #print(type(Goalpoint[0]),' just set to int')
    ...
    axs_astar[0].plot(roboGrid[0], roboGrid[1], "og",markersize=6,label='init_pose')
    ...
    axs_astar[1].plot(roboGrid[0],roboGrid[1],'og',markersize=5)
    ...
    center=np.array([segmented_map.shape[0]/2.,segmented_map.shape[1]/2.])
    goalCenter = Goalpoint-center
    Goalpoint = np.matmul(goalCenter,rotationMatrix(-bestFilter))
    Goalpoint = Goalpoint+center
    Goalpoint = Goalpoint.astype(int)
    
    #Goalpoint.astype(np.int8)
    ...
    #global round_cnt
    #round_cnt += 1
    print(Goalpoint,' the goalpoint that is passed as goalgridx and goalgridy')
    #print(type(Goalpoint[0]),'goalpoint type')
    return ExitRequestResponse(exitpoint[0],exitpoint[1],prob,segmented_OccGrid,heat_OccGrid,int(Goalpoint[1]),int(Goalpoint[0]))
